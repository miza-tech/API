...
use App\Models\Cms\PermissionCategory;
...
class PermissionCategoryController extends ApiController
{
	...
	public function create (Request $request)
	{
		$validator = Validator::make($request->all(), [
			'display_name' => 'required|unique:cms_permission_categories,display_name',
		]);
		if ($validator->fails())
		{
	...
	public function update (Request $request, $id)
	{
		...
		$validator = Validator::make($request->all(), [
			'display_name' => 'required|unique:cms_permission_categories,display_name,' . $category->id,
		]);
		if ($validator->fails())
		{
			return $this->VALIDATOR_FAIL($validator->errors());
